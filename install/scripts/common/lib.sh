...
################################################
# Save a certificate in pem format
function getCredsSecret() {
  local lf_in_ns=$1
  local lf_in_secret_name=$2

  mylog info "getting credeential from $lf_in_secret_name in ns $lf_in_ns"

  cred_user=$(oc -n ${lf_in_ns} get secret ${lf_in_secret_name} -o jsonpath='{.data.username}' | base64 --decode )
  cred_pwd=$(oc -n ${lf_in_ns} get secret ${lf_in_secret_name} -o jsonpath='{.data.password}' | base64 --decode )

}

################################################
# Save a certificate in pem format
function getCredSecret() {
  local lf_in_ns=$1
  local lf_in_secret_name=$2
  local lf_in_cred_path=$3

  mylog info "getting credential from $lf_in_secret_name in ns $lf_in_ns"

  cred_out=$(oc -n ${lf_in_ns} get secret ${lf_in_secret_name} -o jsonpath='{'$lf_in_cred_path'}' | base64 --decode )
